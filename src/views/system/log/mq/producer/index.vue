<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="消费时间" align="center" prop="createTime">
    ...
    <el-dialog :title="title" :visible.sync="open" width="1250px" append-to-body>
      <el-descriptions class="margin-top" title :column="2" size="small" colon border="true">
        <el-descriptions-item label="主键ID">{{form.pkProducerId}}</el-descriptions-item>
        ...
        <el-descriptions-item label="生产时间">{{parseTime(form.createTime)}}</el-descriptions-item>
...
<script>
import { listMqProducer, getMqProducer, delMqProducer } from "@/api/system/log/mq";
export default {
  name: "ProducerMq",
  ...
  methods: {
    ...
    getList () {
      this.loading = true;
      listMqProducer(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
    ...
    /** 查看详情操作 */
    handleDetail (row) {
      const pkProducerId = row.pkProducerId
      getMqProducer(pkProducerId).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "MQ日志：生产日志详情";
      });
    },

    /** 删除按钮操作，多选删除和单个删除 */
    handleDelete (row) {
      const producerIds = row.pkProducerId || this.ids;
      this.$modal.confirm('是否确认删除日志编号为"' + producerIds + '"的数据项？').then(function () {
        return delMqProducer(producerIds);
      }).then(() => {
        this.getList();
